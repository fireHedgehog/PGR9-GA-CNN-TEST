...
def pretty_plot_confusion_matrix(df_cm,
    ...
    # titles and legends
    ax.set_title('Confusion matrix', pad=40)
    ax.set_xlabel(xlbl)
    ax.set_ylabel(ylbl)
...
def plot_confusion_matrix_from_data(y_test,
    ...
    figsize = [11, 11]
    show_null_values = 2
    df_cm = DataFrame(confm, index=[i for i in "0123456789"], columns=[i for i in "0123456789"])
...
def _test_cm():
    ...
    df_cm = DataFrame(array, index=[i for i in "0123456789"],
                      columns=[i for i in "0123456789"])
